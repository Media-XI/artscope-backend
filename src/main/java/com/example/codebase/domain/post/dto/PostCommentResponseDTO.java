...
public class PostCommentResponseDTO {
    ...
    public static PostCommentResponseDTO from(PostComment comment) {
        Long parentId = Optional.ofNullable(comment.getParent())
                .map(PostComment::getId)
                .orElse(null);

        PostCommentResponseDTO response = PostCommentResponseDTO.builder()
                .id(comment.getId())
        ...
        if (comment.getChildComments() != null && !comment.getChildComments().isEmpty()) {
            List<PostCommentResponseDTO> childComments = comment.getChildComments().stream()
